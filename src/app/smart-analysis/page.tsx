...
const LockedContent = () => (
     <div className="relative mt-4 rounded-lg border p-4 text-center">
        <div className="blur-sm select-none">
            <p className="text-muted-foreground">Full analysis including step-by-step breakdowns, chemical details, exam notes, and more is available with a free account.</p>
...
export default function SmartAnalysisPage() {
  ...
  useEffect(() => {
    if (isReady && !isGuest) {
      router.push('/dashboard/smart-analysis');
    }
...
function ImageAnalysisPanel({ title, description, icon, handleFileChange, handleAnalysis, resetState, preview, isLoading, result, error, activeTab, isGuest }: any) {
  return (
    <div className="grid md:grid-cols-2 gap-8 max-w-6xl mx-auto">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            {icon} {title} {isGuest && <Badge variant="outline">Demo Mode</Badge>}
          </CardTitle>
      ...
      <Card className="min-h-full relative">
        {isGuest && result && <Badge variant="outline" className="absolute top-4 right-4 z-10">CuraAI Demo</Badge>}
        <CardHeader>
        ...
        <CardContent>
          ...
          <AnalysisResultDisplay result={result} isGuest={isGuest} />
        </CardContent>
      </Card>
    </div>
  );
...
function TextAnalysisPanel({ handleAnalysis, isLoading, result, error, rawText, setRawText, isGuest }: any) {
    return (
        <div className="grid md:grid-cols-2 gap-8 max-w-6xl mx-auto">
            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                        <FileText /> Text Structuring Engine {isGuest && <Badge variant="outline">Demo Mode</Badge>}
                    </CardTitle>
...
function AnalysisResultDisplay({ result, isGuest }: { result: AnalysisResult | null, isGuest: boolean }) {
    if (!result) return (
    ...
    if (result.textAnalysis) {
        ...
        return (
             <div className="prose prose-sm dark:prose-invert max-w-none whitespace-pre-wrap animate-in fade-in p-1" dangerouslySetInnerHTML={{ __html: res.structuredText.replace(/(\*\*|__)(.*?)\1/g, '<strong>$2</strong>').replace(/\n/g, '<br />') }}>
            </div>
        )
    }
